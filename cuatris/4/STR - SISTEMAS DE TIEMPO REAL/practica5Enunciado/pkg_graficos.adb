...
package body PKG_graficos is
   ...
   BEGIN
      ...
      begin
         -- Buscar posiciÛn del coche dentro del buffer
         Pos := Posicion_Buffer(Coche.Id, Buffer_Coches(Coche.Carril));
   ...
   ------------------------------------------------------------------------
   -- Procedimiento que actualiza el tipo de avería de coche en el carril
   -- especificado
   ------------------------------------------------------------------------
   ...
   BEGIN
      -- Comprobar si se ha comunicado avería en el carril y el coche
      -- es el último dentro del mismo
   ...
   begin
      ...
      -- Dibujar el rótulo de indicación de avería en un coche
      x := 380;
      ...
      -- dibujar fondo para la descripción
      ...
      FOR i IN T_Carril LOOP
         ...
         IF Averia_Carril /= NO_AVERIA THEN
            -- Dibujar el rótulo del tipo de avería sólo cuando ésta exista
         ...
         -- Dibujar el rótulo del estado de la avería
         ...
         -- Dibujar el contador del reloj de la cuenta atrás de la duración de una reparación de avería
         set_text(pangoLayout, Integer'Image(contador_reloj_averia(i)));
         Draw_Layout(Pixmap, Black_Gc, 700, y, pangoLayout);
      END LOOP;

      -- Dibujar la señal de prohibido si un carril está cerrado
      FOR I IN T_Carril LOOP
   ...
   ------------------------------------------------------------------------
   -- Seleccionado Botón 1 para provocar avería en carril 1
   ------------------------------------------------------------------------
   ...
   ------------------------------------------------------------------------
   -- Seleccionado Botón 2 para provocar avería en carril 2
   ------------------------------------------------------------------------
