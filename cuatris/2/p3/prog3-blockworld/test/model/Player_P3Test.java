...
public class Player_P3Test {
	...
	public void testGetOrientation() {
		...
		//Composicion: copia defensiva.
		orientationPlayer = player.getOrientation();
		assertNotSame (orientationPlayer, player.getOrientation());
	...
	public void testUseItemInHand() {
		try {
			...
			//Seleccionamos material de granito y lo usamos 5 veces.
			player.selectItem(2);
			player.useItemInHand(5);
			assertEquals(block, player.useItemInHand(5));
			player.selectItem(1);
			assertEquals(tool, player.useItemInHand(5));
			player.selectItem(0);
			assertEquals(food, player.useItemInHand(4));
			assertNull(player.useItemInHand(1));
			...
			for (int dy=-1; dy<2; dy++)
				for (int dz=-1; dz<2; dz++) {
					if ( (dx!=0)||(dy!=0)||(dz!=0) ) {
						...
						orient = new Location(loc.getWorld(),dx,dy,dz);
						loc.add(orient);
						try {
						...
						assertEquals(loc,player.getOrientation());
					}
				}
	}
