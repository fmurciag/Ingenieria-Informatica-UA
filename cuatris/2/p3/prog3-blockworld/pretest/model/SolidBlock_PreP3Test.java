...
public class SolidBlock_PreP3Test {
...
/* Prueba del constructor y getType con valores correctos. Se usa una variable Block
 * para almacenar un objeto SolidBlock 
 */
@Test
...
public final void testConstructorCopia() {
	...
	try {
		//TODO
		b1= new SolidBlock(Material.GRANITE);
		b2= new SolidBlock(b1);
		 assertTrue(b1.getType() == Material.GRANITE);
		 assertTrue(b1.getDrops() == null);
		assertTrue(b2.getDrops() == null);
		assertTrue(b2.getType() == Material.GRANITE);
		assertEquals(b1, b2);
	} catch (Exception e) {
		fail("Error: excepción "+e.getClass().toString()+" inesperada");
	} 
...
public final void testClone(){
	SolidBlock sb1=null; //Para el bloque a clonar
	Block sb2=null;		 //Para el bloque clonado
	ItemStack item;		//Para el item a añadir a sb1
	try {
		sb1 = new SolidBlock(Material.BEDROCK);
		item= new ItemStack(Material.BEDROCK, 1);
		sb1.setDrops(Material.BEDROCK, 1);
		assertEquals(item, sb1.getDrops());
		sb2=sb1.clone();
